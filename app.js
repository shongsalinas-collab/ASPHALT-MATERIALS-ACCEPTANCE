...
function renderLayingLog() {
  ...
  if (!rows.length) {
    tbody.innerHTML = '<tr class="empty-row"><td colspan="24">No laying entries logged yet. Click "+ Log Laying Entry" to start.</td></tr>';
  } else {
    tbody.innerHTML = rows.map(function(d) {
...
function openAddLayingForm() {
  try {
    ...
    setVal('l-accum-preview',''); setVal('l-duration-preview','');
...
function exportLayingCSV() {
  ...
  const hdrs = ['Date','Lab No.','Project ID','Project Title','Location','Client','JMF','Rate Type','Rate Result',
    ...
    'Quantity (t)','Accumulative Qty (t)','Arrival Time','Laying Start','Laying End','Duration (min)',
