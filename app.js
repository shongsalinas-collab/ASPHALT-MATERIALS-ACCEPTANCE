/* ================================================================
   Big Ben -- Asphalt Materials Acceptance Dashboard
   app.js | Firebase Realtime Database | Offline-first
   ================================================================ */

/* -- PLACEHOLDER: Replace with your Firebase config -- */
const firebaseConfig = {
  apiKey:            "AIzaSyD5BBs9vfhFu1Kw-J7TqTMGJzC-WxTpZyo",
  authDomain:        "bigben-asphalt.firebaseapp.com",
  databaseURL:       "https://bigben-asphalt-default-rtdb.asia-southeast1.firebasedatabase.app",
  projectId:         "bigben-asphalt",
  storageBucket:     "bigben-asphalt.firebasestorage.app",
  messagingSenderId: "390119698156",
  appId:             "1:390119698156:web:14351003894a239f1a3fc9"
};

/* -- Material config -- */
const SUBTYPES = {
  'Coarse Aggregates':   ['Agg - G1"', 'Agg - 3/4"', 'Agg - 3/8"', 'Other'],
  'Fine Aggregates':     ['Sand - S1', 'Sand - 2mm', 'Sand - 4mm', 'Other'],
  'Bituminous Material': ['AC 60/70', 'CSS 1', 'CSS 1H', 'SS 1H', 'MC 70', 'Other'],
  'Mineral Filler':      ['Hydrated Lime', 'Reclaimed Filler', 'Lime Stonedust', 'Other'],
  'Additives':           ['ARA', 'Viatop', 'Other'],
};

const QCP_FREQ = {
  'Coarse Aggregates':   'Every delivery',
  'Fine Aggregates':     'Every delivery',
  'Bituminous Material': 'Every delivery',
  'Mineral Filler':      'Every delivery',
  'Additives':           'Every delivery',
};

/* -- State -- */
let deliveries      = [];
let documents       = [];
let layingEntries   = [];
let editIdx         = null;
let editLayingIdx   = null;
let gaugeCharts     = {};
let trendChart      = null;
let activeTab       = 'kpi';
let db              = null;
let isOnline        = navigator.onLine;
let pendingSync       = [];
let pendingSyncLaying = [];

/* ================================================================
   OFFLINE / ONLINE
   ================================================================ */
function updateOnlineStatus() {
  isOnline = navigator.onLine;
  const el = document.getElementById('online-indicator');
  if (el) {
    el.textContent = isOnline ? 'Online' : 'Offline';
    el.style.background = isOnline ? '#EAF3DE' : '#FAEEDA';
    el.style.color      = isOnline ? '#3B6D11' : '#854F0B';
    el.style.border     = isOnline ? '0.5px solid #639922' : '0.5px solid #EF9F27';
  }
  if (isOnline) syncPending();
}
window.addEventListener('online',  updateOnlineStatus);
window.addEventListener('offline', updateOnlineStatus);

/* ================================================================
   LOCAL STORAGE
   ================================================================ */
const LS_KEY     = 'asphalt_deliveries_local';
const LS_PENDING = 'asphalt_pending_sync';
const LS_KEY_LAYING     = 'asphalt_laying_local';
const LS_PENDING_LAYING = 'asphalt_pending_sync_laying';

function saveLocal() {
  try { localStorage.setItem(LS_KEY, JSON.stringify(deliveries)); } catch(e) {}
}
function loadLocal() {
  try {
    const d = localStorage.getItem(LS_KEY);
    if (d) deliveries = JSON.parse(d);
    const p = localStorage.getItem(LS_PENDING);
    if (p) pendingSync = JSON.parse(p);
  } catch(e) {}
}
function savePending() {
  try { localStorage.setItem(LS_PENDING, JSON.stringify(pendingSync)); } catch(e) {}
}
function clearPending() {
  pendingSync = [];
  try { localStorage.removeItem(LS_PENDING); } catch(e) {}
}

function saveLocalLaying() {
  try { localStorage.setItem(LS_KEY_LAYING, JSON.stringify(layingEntries)); } catch(e) {}
}
function loadLocalLaying() {
  try {
    const d = localStorage.getItem(LS_KEY_LAYING);
    if (d) layingEntries = JSON.parse(d);
    const p = localStorage.getItem(LS_PENDING_LAYING);
    if (p) pendingSyncLaying = JSON.parse(p);
  } catch(e) {}
}
function savePendingLaying() {
  try { localStorage.setItem(LS_PENDING_LAYING, JSON.stringify(pendingSyncLaying)); } catch(e) {}
}
function clearPendingLaying() {
  pendingSyncLaying = [];
  try { localStorage.removeItem(LS_PENDING_LAYING); } catch(e) {}
}

/* ================================================================
   FIREBASE
   ================================================================ */
function initFirebase() {
  try {
    firebase.initializeApp(firebaseConfig);
    db = firebase.database();

    db.ref('deliveries').on('value', snap => {
      const val = snap.val();
      deliveries = val
        ? Object.entries(val).map(([id, d]) => ({ ...d, _id: id }))
        : [];
      deliveries.sort((a, b) =>
        (b.date||'').localeCompare(a.date||'') || (b.time||'').localeCompare(a.time||''));
      saveLocal();
      buildMonthSelect();
      render();
      if (activeTab === 'kpi') setTimeout(renderCharts, 80);
    }, err => {
      console.warn('Firebase error:', err);
      toast('Using offline data', '#854F0B');
    });

    db.ref('documents').on('value', snap => {
      const val = snap.val();
      documents = val
        ? Object.entries(val).map(([id, d]) => ({ ...d, _id: id }))
        : [];
      renderDocs();
      const badge = document.getElementById('doc-count-badge');
      if (badge) badge.textContent = documents.length;
    });

    db.ref('layingData').on('value', snap => {
      const val = snap.val();
      layingEntries = val
        ? Object.entries(val).map(([id, d]) => ({ ...d, _id: id }))
        : [];
      layingEntries.sort((a, b) =>
        (b.date||'').localeCompare(a.date||'') ||
        (a.labNo||'').localeCompare(b.labNo||'') ||
        ((parseFloat(a.loadNo)||0) - (parseFloat(b.loadNo)||0)));
      saveLocalLaying();
      buildLabFilterOptions();
      renderLayingLog();
    }, err => {
      console.warn('Firebase error (layingData):', err);
      toast('Using offline data', '#854F0B');
    });

  } catch(e) {
    console.warn('Firebase init failed:', e);
    toast('Offline mode', '#854F0B');
  }
}

function syncPending() {
  if (!db) return;
  if (pendingSync.length) {
    const toSync = [...pendingSync];
    clearPending();
    toSync.forEach(entry => {
      db.ref('deliveries').push(entry)
        .then(() => toast('Synced: ' + (entry.dr||entry.material), '#639922'))
        .catch(() => { pendingSync.push(entry); savePending(); });
    });
  }
  if (pendingSyncLaying.length) {
    const toSync = [...pendingSyncLaying];
    clearPendingLaying();
    toSync.forEach(entry => {
      db.ref('layingData').push(entry)
        .then(() => toast('Synced: ' + (entry.labNo||entry.projectId||'laying entry'), '#639922'))
        .catch(() => { pendingSyncLaying.push(entry); savePendingLaying(); });
    });
  }
}

/* ================================================================
   SAVE / DELETE
   ================================================================ */
function saveDeliveryToDb(entry) {
  if (!isOnline || !db) {
    const offline = { ...entry, _id: 'offline_' + Date.now(), _pending: true };
    deliveries.unshift(offline);
    saveLocal();
    pendingSync.push(entry);
    savePending();
    return Promise.resolve();
  }
  if (editIdx !== null && deliveries[editIdx] && deliveries[editIdx]._id) {
    return db.ref('deliveries/' + deliveries[editIdx]._id).set(entry);
  }
  return db.ref('deliveries').push(entry);
}

function deleteDeliveryFromDb() {
  if (!isOnline || !db) return Promise.reject(new Error('Offline'));
  return db.ref('deliveries/' + deliveries[editIdx]._id).remove();
}

function saveDocToDb(doc) {
  if (!isOnline || !db) return Promise.reject(new Error('Offline'));
  return db.ref('documents').push(doc);
}

function deleteDocFromDb(id) {
  if (!isOnline || !db) return Promise.reject(new Error('Offline'));
  return db.ref('documents/' + id).remove();
}

function clearAllData() {
  if (!confirm('Clear ALL delivery data? This cannot be undone.')) return;
  if (!isOnline || !db) { toast('Cannot clear while offline', '#E24B4A'); return; }
  db.ref('deliveries').remove()
    .then(() => { localStorage.removeItem(LS_KEY); toast('All data cleared.', '#378ADD'); })
    .catch(err => toast('Error: ' + err.message, '#E24B4A'));
}

function saveLayingToDb(entry) {
  if (!isOnline || !db) {
    const offline = { ...entry, _id: 'offline_' + Date.now(), _pending: true };
    layingEntries.unshift(offline);
    saveLocalLaying();
    pendingSyncLaying.push(entry);
    savePendingLaying();
    return Promise.resolve();
  }
  if (editLayingIdx !== null && layingEntries[editLayingIdx] && layingEntries[editLayingIdx]._id) {
    return db.ref('layingData/' + layingEntries[editLayingIdx]._id).set(entry);
  }
  return db.ref('layingData').push(entry);
}

function deleteLayingFromDb() {
  if (!isOnline || !db) return Promise.reject(new Error('Offline'));
  return db.ref('layingData/' + layingEntries[editLayingIdx]._id).remove();
}

/* ================================================================
   MONTH SELECT
   ================================================================ */
function buildMonthSelect() {
  const sel = document.getElementById('sel-month');
  if (!sel) return;
  const cur = sel.value;
  const months = new Set(deliveries.map(d => (d.date||'').slice(0,7)).filter(Boolean));
  const now = new Date();
  const thisMonth = now.getFullYear() + '-' + String(now.getMonth()+1).padStart(2,'0');
  months.add(thisMonth);
  sel.innerHTML = '';
  const allOpt = document.createElement('option');
  allOpt.value = 'all'; allOpt.textContent = 'All Months';
  if (cur === 'all') allOpt.selected = true;
  sel.appendChild(allOpt);
  [...months].sort().reverse().forEach(m => {
    const [y, mo] = m.split('-');
    const lbl = new Date(+y,+mo-1,1).toLocaleDateString('en-PH',{year:'numeric',month:'long'});
    const o = document.createElement('option');
    o.value = m; o.textContent = lbl;
    if (m === (cur||thisMonth)) o.selected = true;
    sel.appendChild(o);
  });
}

function selMonth() {
  const s = document.getElementById('sel-month');
  return s ? s.value : '';
}
function monthDeliveries() {
  const m = selMonth();
  if (m === 'all') return [...deliveries];
  return deliveries.filter(d => (d.date||'').startsWith(m));
}

/* ================================================================
   TABS
   ================================================================ */
function setTab(name, btn) {
  document.querySelectorAll('.tab').forEach(b => b.classList.remove('active'));
  document.querySelectorAll('.tab-content').forEach(c => c.classList.remove('active'));
  btn.classList.add('active');
  const tab = document.getElementById('tab-' + name);
  if (tab) tab.classList.add('active');
  activeTab = name;
  if (name === 'kpi') setTimeout(renderCharts, 80);
  if (name === 'docs') renderDocs();
  if (name === 'laying') renderLayingLog();
}

/* ================================================================
   RENDER
   ================================================================ */
function render() {
  const now = new Date();
  setEl('cur-date', now.toLocaleDateString('en-PH',
    {weekday:'short',year:'numeric',month:'short',day:'numeric'}));

  const md = monthDeliveries();
  const [y, mo] = (selMonth()||'').split('-');
  const lbl = y && mo
    ? new Date(+y,+mo-1,1).toLocaleDateString('en-PH',{year:'numeric',month:'long'})
    : '';
  setEl('log-month-label', lbl);
  setEl('trend-month-label', lbl);

  renderOverall(md);
  renderGaugeType(md);
  renderGaugeSubtype(md);
  renderLog();
}

function setEl(id, val) {
  const el = document.getElementById(id);
  if (el) el.textContent = val;
}

/* -- Overall KPI -- */
function renderOverall(md) {
  const tot  = md.length;
  const pass = md.filter(d => d.status==='Passed').length;
  const rej  = md.filter(d => d.status==='Rejected').length;
  const pend = md.filter(d => d.status==='Pending').length;
  const pct  = tot ? Math.round(pass/tot*100) : 0;

  const color    = pct===100?'#3B6D11':pct>=80?'#854F0B':'#A32D2D';
  const barColor = pct===100?'#639922':pct>=80?'#EF9F27':'#E24B4A';

  const pctEl = document.querySelector('#overall-card .overall-big span:first-child');
  if (pctEl) { pctEl.textContent = pct+'%'; pctEl.style.color = color; }

  const bar = document.getElementById('overall-bar');
  if (bar) { bar.style.width = pct+'%'; bar.style.background = barColor; }

  const noteEl = document.getElementById('overall-note');
  if (noteEl) noteEl.textContent = pct===100
    ? 'All '+tot+' deliveries passed -- monthly target achieved!'
    : (100-pct)+'% gap to 100% target - '+rej+' rejection'+(rej!==1?'s':'')+' this month'+(pend?' - '+pend+' pending':'');

  ['mini-total','mini-pass','mini-rej','mini-pend'].forEach((id, i) => {
    const el = document.querySelector('#'+id+' .ms-val');
    if (el) el.textContent = [tot,pass,rej,pend][i];
  });
}

/* -- Gauges per type -- */
function renderGaugeType(md) {
  const grid = document.getElementById('gauge-type-grid');
  if (!grid) return;
  grid.innerHTML = '';
  Object.keys(SUBTYPES).forEach(function(mat, i) {
    const rows = md.filter(d => d.material===mat);
    const tot  = rows.length;
    const pass = rows.filter(d => d.status==='Passed').length;
    const pct  = tot ? Math.round(pass/tot*100) : null;
    const color     = pct===null?'#999':pct===100?'#3B6D11':pct>=80?'#854F0B':'#A32D2D';
    const fillColor = pct===null?'#ddd':pct===100?'#639922':pct>=80?'#EF9F27':'#E24B4A';
    const cls       = pct===null?'':pct===100?'hit':pct>=80?'warn':'critical';
    const pillCls   = pct===null?'p-pend':pct===100?'p-pass':pct>=80?'p-pend':'p-rej';
    const pillLbl   = pct===null?'no data':pct===100?'on target':pct>=80?'below target':'critical';

    const card = document.createElement('div');
    card.className = 'gauge-card '+cls;
    card.innerHTML =
      '<div class="gauge-mat">'+mat+'</div>'+
      '<div class="gauge-wrap"><canvas id="gc-'+i+'" width="90" height="50"></canvas></div>'+
      '<div class="gauge-pct" style="color:'+color+'">'+(pct!==null?pct+'%':'--')+'</div>'+
      '<div class="gauge-det">'+pass+'/'+tot+' passed</div>'+
      '<span class="gauge-pill badge '+pillCls+'">'+pillLbl+'</span>';
    grid.appendChild(card);

    setTimeout(function() {
      const ctx = document.getElementById('gc-'+i);
      if (!ctx) return;
      if (gaugeCharts[i]) { try { gaugeCharts[i].destroy(); } catch(e){} }
      gaugeCharts[i] = new Chart(ctx, {
        type:'doughnut',
        data:{ datasets:[{ data:[pct||0,100-(pct||0)],
          backgroundColor:[fillColor,'rgba(128,128,128,0.1)'],
          borderWidth:0, circumference:180, rotation:270 }]},
        options:{ responsive:false, maintainAspectRatio:false, cutout:'68%',
          plugins:{legend:{display:false},tooltip:{enabled:false}},
          animation:{duration:500}}
      });
    }, 100+i*30);
  });
}

/* -- Gauges per subtype -- */
function renderGaugeSubtype(md) {
  const grid = document.getElementById('gauge-subtype-grid');
  if (!grid) return;
  grid.innerHTML = '';
  Object.entries(SUBTYPES).forEach(function([mat, subs]) {
    if (!subs.length) return;
    subs.forEach(function(sub) {
      const rows = md.filter(d => d.material===mat && (d.type===sub||d.subtype===sub));
      const tot  = rows.length;
      const pass = rows.filter(d => d.status==='Passed').length;
      const pct  = tot ? Math.round(pass/tot*100) : null;
      const color     = pct===null?'#999':pct===100?'#3B6D11':pct>=80?'#854F0B':'#A32D2D';
      const fillColor = pct===null?'#eee':pct===100?'#639922':pct>=80?'#EF9F27':'#E24B4A';
      const pillCls   = pct===null?'p-pend':pct===100?'p-pass':pct>=80?'p-pend':'p-rej';

      const card = document.createElement('div');
      card.className = 'sub-card';
      card.innerHTML =
        '<div class="sub-mat">'+mat+'</div>'+
        '<div class="sub-name">'+sub+'</div>'+
        '<div class="sub-pct" style="color:'+color+'">'+(pct!==null?pct+'%':'--')+'</div>'+
        '<div class="sub-det">'+pass+'/'+tot+' passed '+
        '<span class="pill '+pillCls+'" style="margin-left:3px">'+(pct===null?'no data':pct===100?'on target':pct>=80?'below':'critical')+'</span></div>'+
        '<div class="prog-bar"><div class="prog-fill" style="width:'+(pct||0)+'%;background:'+fillColor+'"></div></div>';
      grid.appendChild(card);
    });
  });
}

/* -- Trend chart -- */
function renderCharts() {
  const md = monthDeliveries();
  const monthStr = selMonth();
  if (!monthStr) return;
  if (trendChart) { try { trendChart.destroy(); } catch(e){} trendChart = null; }
  const [y, mo] = monthStr.split('-');
  const days = Array.from({length:new Date(+y,+mo,0).getDate()},(_,i) =>
    monthStr+'-'+String(i+1).padStart(2,'0'));
  const passRates = days.map(date => {
    const rows = md.filter(d => d.date===date && d.status!=='Pending');
    if (!rows.length) return null;
    return Math.round(rows.filter(d => d.status==='Passed').length/rows.length*100);
  });
  const rejCounts = days.map(date =>
    md.filter(d => d.date===date && d.status==='Rejected').length);
  const ctx = document.getElementById('chartTrend');
  if (!ctx) return;
  trendChart = new Chart(ctx, {
    type:'bar',
    data:{ labels:days.map(d=>d.slice(8)), datasets:[
      {type:'line',data:passRates,borderColor:'#3B6D11',backgroundColor:'rgba(59,109,17,0.07)',
       tension:0.35,fill:true,pointRadius:3,pointBackgroundColor:'#639922',
       borderWidth:2,spanGaps:true,yAxisID:'y'},
      {type:'line',data:days.map(()=>100),borderColor:'#378ADD',borderDash:[5,4],
       pointRadius:0,fill:false,borderWidth:1.5,yAxisID:'y'},
      {type:'bar',data:rejCounts,backgroundColor:'rgba(226,75,74,0.28)',
       borderRadius:2,yAxisID:'y2'}
    ]},
    options:{
      responsive:true, maintainAspectRatio:false,
      plugins:{legend:{display:false}},
      scales:{
        x:{grid:{display:false},ticks:{font:{size:9},autoSkip:true,maxTicksLimit:15}},
        y:{min:0,max:105,position:'left',
          ticks:{font:{size:9},callback:v=>v+'%'},grid:{color:'rgba(128,128,128,0.07)'}},
        y2:{min:0,max:6,position:'right',
          ticks:{font:{size:9},stepSize:1},grid:{display:false}}
      }
    }
  });
}

/* -- Delivery Log -- */
function renderLog() {
  const md     = monthDeliveries();
  const search = (document.getElementById('search-box')||{value:''}).value.toLowerCase();
  const fMat   = (document.getElementById('filter-mat')||{value:''}).value;
  const fStat  = (document.getElementById('filter-status')||{value:''}).value;

  let rows = [...md].sort((a,b) =>
    (b.date||'').localeCompare(a.date||'')||(b.time||'').localeCompare(a.time||''));
  if (search) rows = rows.filter(r =>
    [r.supplier,r.material,r.type,r.dr,r.plate,r.source,r.tester,r.remarks]
    .join(' ').toLowerCase().includes(search));
  if (fMat)  rows = rows.filter(r => r.material===fMat);
  if (fStat) rows = rows.filter(r => r.status===fStat);

  const tbody = document.getElementById('log-body');
  if (!tbody) return;

  if (!rows.length) {
    tbody.innerHTML = '<tr class="empty-row"><td colspan="12">No deliveries logged yet. Click "+ Log Delivery" to start.</td></tr>';
  } else {
    tbody.innerHTML = rows.map(function(d) {
      const idx = deliveries.findIndex(x => x._id===d._id);
      const pc  = d.status==='Passed'?'p-pass':d.status==='Rejected'?'p-rej':'p-pend';
      const op  = d._pending?' style="opacity:0.7"':'';
      return '<tr'+op+'>'+
        '<td>'+(d.date||'--')+'</td>'+
        '<td>'+(d.time||'--')+'</td>'+
        '<td title="'+(d.dr||'')+'">'+(d.dr||'--')+'</td>'+
        '<td title="'+(d.plate||'')+'">'+(d.plate||'--')+'</td>'+
        '<td title="'+(d.supplier||'')+'">'+(d.supplier||'--')+'</td>'+
        '<td title="'+(d.material||'')+'">'+(d.material||'--')+'</td>'+
        '<td title="'+(d.type||d.subtype||'')+'">'+(d.type||d.subtype||'--')+'</td>'+
        '<td title="'+(d.source||'')+'">'+(d.source||'--')+'</td>'+
        '<td><span class="pill '+pc+'">'+d.status+'</span>'+
          (d._pending?'<span style="font-size:9px;color:#854F0B;margin-left:3px">pending</span>':'')+
        '</td>'+
        '<td>'+(d.tester||'--')+'</td>'+
        '<td title="'+(d.remarks||'')+'">'+(d.remarks||'--')+'</td>'+
        '<td>'+(d._pending?'':
          '<button class="act-btn" onclick="openEditForm('+idx+')" title="Edit">'+
          '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">'+
          '<path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/>'+
          '<path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/>'+
          '</svg></button>')+
        '</td>'+
      '</tr>';
    }).join('');
  }
  const footer = document.getElementById('log-footer');
  if (footer) footer.textContent = 'Showing '+rows.length+' of '+md.length+' entries for this month';
}

/* -- Asphalt Laying Data -- */
function computeDurationMinutes(start, end) {
  if (!start || !end) return null;
  const sp = start.split(':').map(Number);
  const ep = end.split(':').map(Number);
  if (sp.length<2 || ep.length<2 || sp.some(isNaN) || ep.some(isNaN)) return null;
  let mins = (ep[0]*60+ep[1]) - (sp[0]*60+sp[1]);
  if (mins < 0) mins += 24*60;
  return mins;
}

function buildAccumMap(entries) {
  const map = {};
  const groups = {};
  entries.forEach(e => {
    const k = e.labNo || '';
    (groups[k] = groups[k]||[]).push(e);
  });
  Object.values(groups).forEach(list => {
    list.sort((a,b) =>
      (parseFloat(a.loadNo)||0)-(parseFloat(b.loadNo)||0) ||
      (a.date||'').localeCompare(b.date||''));
    let sum = 0;
    list.forEach(e => {
      sum += parseFloat(e.quantity)||0;
      if (e._id) map[e._id] = sum;
    });
  });
  return map;
}

function buildAccumDurationMap(entries) {
  const map = {};
  const groups = {};
  entries.forEach(e => {
    const k = e.labNo || '';
    (groups[k] = groups[k]||[]).push(e);
  });
  Object.values(groups).forEach(list => {
    list.sort((a,b) =>
      (parseFloat(a.loadNo)||0)-(parseFloat(b.loadNo)||0) ||
      (a.date||'').localeCompare(b.date||''));
    let sum = 0;
    list.forEach(e => {
      const mins = computeDurationMinutes(e.layingStart, e.layingEnd) || 0;
      sum += mins;
      if (e._id) map[e._id] = sum;
    });
  });
  return map;
}

function buildLabFilterOptions() {
  const sel = document.getElementById('laying-filter-lab');
  if (!sel) return;
  const cur = sel.value;
  const labs = [...new Set(layingEntries.map(d=>d.labNo).filter(Boolean))].sort();
  sel.innerHTML = '<option value="">All Lab Nos.</option>' +
    labs.map(l => '<option'+(l===cur?' selected':'')+'>'+l+'</option>').join('');
}

function renderLayingLog() {
  const search  = (document.getElementById('laying-search')||{value:''}).value.toLowerCase();
  const fLab    = (document.getElementById('laying-filter-lab')||{value:''}).value;
  const fLayer  = (document.getElementById('laying-filter-layer')||{value:''}).value;

  let rows = [...layingEntries];
  if (search) rows = rows.filter(r =>
    [r.labNo,r.projectId,r.projectTitle,r.projectLocation,r.client,r.truckNo,r.stationFrom,r.stationTo]
    .join(' ').toLowerCase().includes(search));
  if (fLab)   rows = rows.filter(r => r.labNo===fLab);
  if (fLayer) rows = rows.filter(r => (r.layer===fLayer) || (r.layer==='__other__' && r.layerOther===fLayer));

  const accumMap    = buildAccumMap(layingEntries);
  const accumDurMap = buildAccumDurationMap(layingEntries);

  const tbody = document.getElementById('laying-body');
  if (!tbody) return;

  if (!rows.length) {
    tbody.innerHTML = '<tr class="empty-row"><td colspan="25">No laying entries logged yet. Click "+ Log Laying Entry" to start.</td></tr>';
  } else {
    tbody.innerHTML = rows.map(function(d) {
      const idx = layingEntries.findIndex(x => x._id===d._id);
      const accum    = accumMap[d._id] !== undefined ? accumMap[d._id].toFixed(2) : '--';
      const accumDur = accumDurMap[d._id] !== undefined ? accumDurMap[d._id] : '--';
      const dur     = computeDurationMinutes(d.layingStart, d.layingEnd);
      const jmfLbl  = d.jmf === '__other__' ? (d.jmfOther||'Other') : (d.jmf||'--');
      const laneLbl = d.laneArea === '__other__' ? (d.laneAreaOther||'Other') : (d.laneArea||'--');
      const layerLbl= d.layer === '__other__' ? (d.layerOther||'Other') : (d.layer||'--');
      const rateLbl = (d.rateType||d.rateResult) ? ((d.rateType||'')+(d.rateResult?': '+d.rateResult:'')) : '--';
      const tempLbl = [d.tempArrival,d.tempLaying,d.tempCompaction].map(t=> (t!==undefined&&t!==null&&t!=='') ? t : '-').join('/') +
        (d.tempOthersLabel ? ' ('+d.tempOthersLabel+': '+(d.tempOthersValue||'-')+')' : '');
      const op = d._pending?' style="opacity:0.7"':'';
      return '<tr'+op+'>'+
        '<td>'+(d.date||'--')+'</td>'+
        '<td>'+(d.labNo||'--')+'</td>'+
        '<td>'+(d.projectId||'--')+'</td>'+
        '<td>'+(d.projectTitle||'--')+'</td>'+
        '<td>'+(d.projectLocation||'--')+'</td>'+
        '<td>'+(d.client||'--')+'</td>'+
        '<td>'+jmfLbl+'</td>'+
        '<td>'+rateLbl+'</td>'+
        '<td>'+(d.loadNo||'--')+'</td>'+
        '<td>'+(d.truckNo||'--')+'</td>'+
        '<td>'+((d.stationFrom||'--')+' - '+(d.stationTo||'--'))+'</td>'+
        '<td>'+(d.length||'--')+'</td>'+
        '<td>'+(d.width||'--')+'</td>'+
        '<td>'+(d.thickness||'--')+'</td>'+
        '<td>'+(d.laneNo||'--')+'</td>'+
        '<td>'+laneLbl+'</td>'+
        '<td>'+(d.quantity||'--')+'</td>'+
        '<td>'+accum+'</td>'+
        '<td>'+(d.arrivalTime||'--')+'</td>'+
        '<td>'+((d.layingStart||'--')+' - '+(d.layingEnd||'--'))+'</td>'+
        '<td>'+(dur!==null?dur:'--')+'</td>'+
        '<td>'+accumDur+'</td>'+
        '<td>'+tempLbl+'</td>'+
        '<td>'+layerLbl+'</td>'+
        '<td>'+(d._pending?'':
          '<button class="act-btn" onclick="openEditLayingForm('+idx+')" title="Edit">'+
          '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">'+
          '<path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/>'+
          '<path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/>'+
          '</svg></button>')+
        '</td>'+
      '</tr>';
    }).join('');
  }
  const footer = document.getElementById('laying-footer');
  if (footer) footer.textContent = 'Showing '+rows.length+' of '+layingEntries.length+' entries';
}

/* ================================================================
   HELPERS
   ================================================================ */
function getVal(id) { const el=document.getElementById(id); return el?el.value:''; }
function setVal(id,v){ const el=document.getElementById(id); if(el) el.value=v; }
function openModal(id){ const el=document.getElementById(id); if(el) el.classList.add('open'); }
function closeModal(id){ const el=document.getElementById(id); if(el) el.classList.remove('open'); }

function updateSubtypes(selSub) {
  const mat  = getVal('m-material');
  const subs = SUBTYPES[mat]||[];
  const sel  = document.getElementById('m-subtype');
  if (!sel) return;
  if (!subs.length) {
    sel.innerHTML = '<option value="">n/a (input not needed)</option>';
  } else {
    sel.innerHTML = '<option value="">-- Select --</option>';
    subs.forEach(s => {
      const o = document.createElement('option');
      o.value=s; o.textContent=s;
      if (s===selSub) o.selected=true;
      sel.appendChild(o);
    });
  }
}

function toggleLOther(selId, wrapId) {
  const sel  = document.getElementById(selId);
  const wrap = document.getElementById(wrapId);
  if (!sel || !wrap) return;
  wrap.style.display = sel.value === '__other__' ? '' : 'none';
}

function updateAccumPreview() {
  const labNo  = getVal('l-labno');
  const loadNo = getVal('l-loadno');
  const qty    = parseFloat(getVal('l-qty'))||0;
  const curId  = (editLayingIdx!==null && layingEntries[editLayingIdx]) ? layingEntries[editLayingIdx]._id : null;

  let list = layingEntries
    .filter(e => (e.labNo||'')===labNo && e._id !== curId)
    .map(e => ({ loadNo: e.loadNo, quantity: e.quantity }));
  list.push({ loadNo, quantity: qty });
  list.sort((a,b) => (parseFloat(a.loadNo)||0)-(parseFloat(b.loadNo)||0));

  let sum = 0, result = 0;
  list.forEach(e => {
    sum += parseFloat(e.quantity)||0;
    if (String(e.loadNo)===String(loadNo)) result = sum;
  });
  setVal('l-accum-preview', labNo ? result.toFixed(2)+' t' : '--');
}

function updateDurationPreview() {
  const mins = computeDurationMinutes(getVal('l-start'), getVal('l-end'));
  setVal('l-duration-preview', mins!==null ? mins+' min' : '--');
  updateAccumDurationPreview(mins);
}

function updateAccumDurationPreview(currentMins) {
  const labNo  = getVal('l-labno');
  const loadNo = getVal('l-loadno');
  if (currentMins === undefined) {
    currentMins = computeDurationMinutes(getVal('l-start'), getVal('l-end'));
  }
  const curId = (editLayingIdx!==null && layingEntries[editLayingIdx]) ? layingEntries[editLayingIdx]._id : null;

  let list = layingEntries
    .filter(e => (e.labNo||'')===labNo && e._id !== curId)
    .map(e => ({ loadNo: e.loadNo, mins: computeDurationMinutes(e.layingStart, e.layingEnd) || 0 }));
  list.push({ loadNo, mins: currentMins || 0 });
  list.sort((a,b) => (parseFloat(a.loadNo)||0)-(parseFloat(b.loadNo)||0));

  let sum = 0, result = 0;
  list.forEach(e => {
    sum += e.mins || 0;
    if (String(e.loadNo)===String(loadNo)) result = sum;
  });
  setVal('l-accum-duration-preview', labNo ? result+' min' : '--');
}

/* ================================================================
   DELIVERY MODAL
   ================================================================ */
function openAddForm() {
  try {
    editIdx = null;
    const now = new Date();
    const title = document.getElementById('modal-title');
    if (title) title.textContent = 'Log New Delivery';
    const delBtn = document.getElementById('modal-delete-btn');
    if (delBtn) delBtn.style.display = 'none';
    const saveBtn = document.getElementById('modal-save-btn');
    if (saveBtn) saveBtn.textContent = 'Save Delivery';

    setVal('m-date', now.toISOString().split('T')[0]);
    setVal('m-time', now.toTimeString().slice(0,5));
    ['m-dr','m-plate','m-supplier','m-source','m-tester','m-remarks']
      .forEach(id => setVal(id,''));
    setVal('m-material','');
    setVal('m-status','Passed');
    setVal('m-status-other','');
    setVal('m-subtype-other','');
    const sw=document.getElementById('status-other-wrap'); if(sw) sw.style.display='none';
    const tw=document.getElementById('type-other-wrap'); if(tw) tw.style.display='none';
    updateSubtypes();
    openModal('delivery-modal');
  } catch(e) {
    console.error('openAddForm:', e);
    toast('Error: '+e.message,'#E24B4A');
  }
}

function openEditForm(idx) {
  try {
    editIdx = idx;
    const d = deliveries[idx];
    if (!d) return;
    const title = document.getElementById('modal-title');
    if (title) title.textContent = 'Edit Delivery Entry';
    const delBtn = document.getElementById('modal-delete-btn');
    if (delBtn) delBtn.style.display = 'inline-flex';
    const saveBtn = document.getElementById('modal-save-btn');
    if (saveBtn) saveBtn.textContent = 'Save Changes';

    setVal('m-date',     d.date||'');
    setVal('m-time',     d.time||'');
    setVal('m-dr',       d.dr||'');
    setVal('m-plate',    d.plate||'');
    setVal('m-supplier', d.supplier||'');
    setVal('m-material', d.material||'');
    updateSubtypes(d.type||d.subtype||'');
    setVal('m-source',   d.source||'');
    setVal('m-status',   d.status||'Passed');
    setVal('m-tester',   d.tester||'');
    setVal('m-remarks',  d.remarks||'');
    openModal('delivery-modal');
  } catch(e) {
    console.error('openEditForm:', e);
    toast('Error: '+e.message,'#E24B4A');
  }
}

function toggleTypeOther() {
  const sel = document.getElementById('m-subtype');
  const wrap = document.getElementById('type-other-wrap');
  if (!sel || !wrap) return;
  const val = sel.value;
  wrap.style.display = val === 'Other' ? '' : 'none';
}

function toggleStatusOther() {
  const wrap = document.getElementById('status-other-wrap');
  if (wrap) wrap.style.display = getVal('m-status') === '__other__' ? '' : 'none';
}

function closeDeliveryModal() {
  closeModal('delivery-modal');
  editIdx = null;
}

function saveDelivery() {
  const date     = getVal('m-date');
  const supplier = getVal('m-supplier').trim();
  const material = getVal('m-material');
  if (!date||!supplier||!material) {
    toast('Please fill in Date, Supplier and Material.','#E24B4A');
    return;
  }
  const entry = {
    date, time:getVal('m-time'),
    dr:getVal('m-dr').trim(), plate:getVal('m-plate').trim(),
    supplier, material,
    type:getVal('m-subtype'), source:getVal('m-source').trim(),
    status:getVal('m-status'), tester:getVal('m-tester').trim(),
    remarks:getVal('m-remarks').trim(),
  };
  const btn = document.getElementById('modal-save-btn');
  if (btn) { btn.textContent='Saving...'; btn.disabled=true; }

  saveDeliveryToDb(entry)
    .then(() => {
      closeDeliveryModal();
      setVal('sel-month', entry.date.slice(0,7));
      buildMonthSelect();
      render();
      if (activeTab==='kpi') setTimeout(renderCharts,80);
      toast(editIdx!==null?'Entry updated!':'Delivery logged: '+(entry.dr||entry.material),
        entry.status==='Passed'?'#639922':'#E24B4A');
    })
    .catch(err => toast('Save failed: '+err.message,'#E24B4A'))
    .finally(() => {
      if (btn) { btn.textContent=editIdx!==null?'Save Changes':'Save Delivery'; btn.disabled=false; }
    });
}

function deleteEntry() {
  if (editIdx===null) return;
  if (!confirm('Delete this delivery entry? This cannot be undone.')) return;
  deleteDeliveryFromDb()
    .then(() => { closeDeliveryModal(); toast('Entry deleted.','#E24B4A'); })
    .catch(err => toast('Delete failed: '+err.message,'#E24B4A'));
}

/* ================================================================
   ASPHALT LAYING INLINE FORM (collapsible card, not a modal)
   ================================================================ */
function toggleLayingFormPanel() {
  const body = document.getElementById('laying-form-body');
  if (!body) return;
  body.classList.toggle('open');
}

function openAddLayingForm() {
  try {
    editLayingIdx = null;
    const now = new Date();
    const title = document.getElementById('laying-modal-title');
    if (title) title.textContent = 'Log New Asphalt Laying Entry';
    const delBtn = document.getElementById('laying-delete-btn');
    if (delBtn) delBtn.style.display = 'none';
    const saveBtn = document.getElementById('laying-save-btn');
    if (saveBtn) saveBtn.textContent = 'Save Entry';

    setVal('l-date', now.toISOString().split('T')[0]);
    ['l-labno','l-projid','l-projtitle','l-projloc','l-client','l-jmf-other','l-rate-result',
     'l-loadno','l-truckno','l-stafrom','l-stato','l-length','l-width','l-thickness',
     'l-laneno','l-lanearea-other','l-qty','l-arrival','l-start','l-end',
     'l-temp-arrival','l-temp-laying','l-temp-compaction','l-temp-others-label','l-temp-others-value',
     'l-layer-other'].forEach(id => setVal(id,''));
    setVal('l-jmf',''); setVal('l-rate-type',''); setVal('l-lanearea',''); setVal('l-layer','');
    ['l-jmf-other-wrap','l-lanearea-other-wrap','l-layer-other-wrap'].forEach(id=>{
      const w=document.getElementById(id); if(w) w.style.display='none';
    });
    setVal('l-accum-preview',''); setVal('l-duration-preview',''); setVal('l-accum-duration-preview','');

    const body = document.getElementById('laying-form-body');
    if (body) body.classList.add('open');
    const card = document.getElementById('laying-form-card');
    if (card) card.scrollIntoView({behavior:'smooth', block:'start'});
  } catch(e) {
    console.error('openAddLayingForm:', e);
    toast('Error: '+e.message,'#E24B4A');
  }
}

function openEditLayingForm(idx) {
  try {
    editLayingIdx = idx;
    const d = layingEntries[idx];
    if (!d) return;
    const title = document.getElementById('laying-modal-title');
    if (title) title.textContent = 'Edit Asphalt Laying Entry';
    const delBtn = document.getElementById('laying-delete-btn');
    if (delBtn) delBtn.style.display = 'inline-flex';
    const saveBtn = document.getElementById('laying-save-btn');
    if (saveBtn) saveBtn.textContent = 'Save Changes';

    setVal('l-date', d.date||''); setVal('l-labno', d.labNo||''); setVal('l-projid', d.projectId||'');
    setVal('l-projtitle', d.projectTitle||''); setVal('l-projloc', d.projectLocation||''); setVal('l-client', d.client||'');
    setVal('l-jmf', d.jmf||''); setVal('l-jmf-other', d.jmfOther||'');
    const jw = document.getElementById('l-jmf-other-wrap'); if (jw) jw.style.display = d.jmf==='__other__'?'':'none';
    setVal('l-rate-type', d.rateType||''); setVal('l-rate-result', d.rateResult||'');
    setVal('l-loadno', d.loadNo||''); setVal('l-truckno', d.truckNo||'');
    setVal('l-stafrom', d.stationFrom||''); setVal('l-stato', d.stationTo||'');
    setVal('l-length', d.length||''); setVal('l-width', d.width||''); setVal('l-thickness', d.thickness||'');
    setVal('l-laneno', d.laneNo||''); setVal('l-lanearea', d.laneArea||''); setVal('l-lanearea-other', d.laneAreaOther||'');
    const lw = document.getElementById('l-lanearea-other-wrap'); if (lw) lw.style.display = d.laneArea==='__other__'?'':'none';
    setVal('l-qty', d.quantity||'');
    setVal('l-arrival', d.arrivalTime||''); setVal('l-start', d.layingStart||''); setVal('l-end', d.layingEnd||'');
    setVal('l-temp-arrival', d.tempArrival||''); setVal('l-temp-laying', d.tempLaying||'');
    setVal('l-temp-compaction', d.tempCompaction||''); setVal('l-temp-others-label', d.tempOthersLabel||'');
    setVal('l-temp-others-value', d.tempOthersValue||'');
    setVal('l-layer', d.layer||''); setVal('l-layer-other', d.layerOther||'');
    const yw = document.getElementById('l-layer-other-wrap'); if (yw) yw.style.display = d.layer==='__other__'?'':'none';

    updateAccumPreview();
    updateDurationPreview();

    const body = document.getElementById('laying-form-body');
    if (body) body.classList.add('open');
    const card = document.getElementById('laying-form-card');
    if (card) card.scrollIntoView({behavior:'smooth', block:'start'});
  } catch(e) {
    console.error('openEditLayingForm:', e);
    toast('Error: '+e.message,'#E24B4A');
  }
}

function closeLayingModal() {
  const body = document.getElementById('laying-form-body');
  if (body) body.classList.remove('open');
  editLayingIdx = null;
}

function saveLayingEntry() {
  const date  = getVal('l-date');
  const labNo = getVal('l-labno').trim();
  if (!date) {
    toast('Please fill in Date of Laying.','#E24B4A');
    return;
  }
  const entry = {
    date, labNo,
    projectId:getVal('l-projid').trim(), projectTitle:getVal('l-projtitle').trim(),
    projectLocation:getVal('l-projloc').trim(), client:getVal('l-client').trim(),
    jmf:getVal('l-jmf'), jmfOther:getVal('l-jmf-other').trim(),
    rateType:getVal('l-rate-type'), rateResult:getVal('l-rate-result').trim(),
    loadNo:getVal('l-loadno').trim(), truckNo:getVal('l-truckno').trim(),
    stationFrom:getVal('l-stafrom').trim(), stationTo:getVal('l-stato').trim(),
    length:getVal('l-length'), width:getVal('l-width'), thickness:getVal('l-thickness'),
    laneNo:getVal('l-laneno').trim(), laneArea:getVal('l-lanearea'), laneAreaOther:getVal('l-lanearea-other').trim(),
    quantity:getVal('l-qty'),
    arrivalTime:getVal('l-arrival'), layingStart:getVal('l-start'), layingEnd:getVal('l-end'),
    tempArrival:getVal('l-temp-arrival'), tempLaying:getVal('l-temp-laying'), tempCompaction:getVal('l-temp-compaction'),
    tempOthersLabel:getVal('l-temp-others-label').trim(), tempOthersValue:getVal('l-temp-others-value'),
    layer:getVal('l-layer'), layerOther:getVal('l-layer-other').trim(),
  };
  const btn = document.getElementById('laying-save-btn');
  if (btn) { btn.textContent='Saving...'; btn.disabled=true; }

  saveLayingToDb(entry)
    .then(() => {
      closeLayingModal();
      buildLabFilterOptions();
      renderLayingLog();
      toast(editLayingIdx!==null?'Entry updated!':'Laying entry logged: '+(labNo||date), '#639922');
    })
    .catch(err => toast('Save failed: '+err.message,'#E24B4A'))
    .finally(() => {
      if (btn) { btn.textContent = editLayingIdx!==null?'Save Changes':'Save Entry'; btn.disabled=false; }
    });
}

function deleteLayingEntry() {
  if (editLayingIdx===null) return;
  if (!confirm('Delete this laying entry? This cannot be undone.')) return;
  deleteLayingFromDb()
    .then(() => { closeLayingModal(); toast('Entry deleted.','#E24B4A'); })
    .catch(err => toast('Delete failed: '+err.message,'#E24B4A'));
}

/* ================================================================
   DOCUMENTS
   ================================================================ */
function openDocForm() {
  setVal('df-date', new Date().toISOString().split('T')[0]);
  ['df-title','df-issuer','df-link','df-notes'].forEach(id => setVal(id,''));
  const fi = document.getElementById('df-file');
  if (fi) fi.value = '';
  const fp = document.getElementById('doc-form-panel');
  if (fp) fp.classList.add('open');
}
function closeDocForm() {
  const fp = document.getElementById('doc-form-panel');
  if (fp) fp.classList.remove('open');
}

function saveDoc() {
  const title = getVal('df-title').trim();
  if (!title) { toast('Please enter a document title.','#E24B4A'); return; }
  const fi = document.getElementById('df-file');
  const file = fi ? fi.files[0] : null;

  const finalize = (fileData, fileName, fileType) => {
    saveDocToDb({
      title, material:getVal('df-mat'), doctype:getVal('df-type'),
      date:getVal('df-date'), issuer:getVal('df-issuer').trim(),
      link:getVal('df-link').trim(), notes:getVal('df-notes').trim(),
      fileData:fileData||null, fileName:fileName||null, fileType:fileType||null,
      savedAt:Date.now(),
    }).then(() => { closeDocForm(); toast('Document saved.','#639922'); })
      .catch(err => toast('Save failed: '+err.message,'#E24B4A'));
  };

  if (file) {
    if (file.size>5*1024*1024) { toast('File too large - max 5MB.','#E24B4A'); return; }
    const r = new FileReader();
    r.onload = e => finalize(e.target.result, file.name, file.type);
    r.readAsDataURL(file);
  } else {
    finalize(null,null,null);
  }
}

function renderDocs() {
  const grid = document.getElementById('doc-grid');
  if (!grid) return;
  if (!documents.length) {
    grid.innerHTML = '<div class="no-docs">No documents yet. Click "+ Add Document" to attach quality tests.</div>';
    return;
  }
  grid.innerHTML = documents.map(doc =>
    '<div class="doc-card">'+
      '<div class="doc-card-head"><div>'+
        '<div class="doc-title">'+doc.title+'</div>'+
        '<div class="doc-meta">'+
          '<span class="doc-tag">'+doc.material+'</span>'+
          '<span class="doc-tag">'+doc.doctype+'</span>'+
          (doc.date?'<span>'+doc.date+'</span>':'')+
          (doc.issuer?'<span>- '+doc.issuer+'</span>':'')+
        '</div></div></div>'+
      (doc.notes?'<div class="doc-notes">'+doc.notes+'</div>':'')+
      '<div class="doc-actions">'+
        (doc.fileData?
          '<a class="doc-link-btn" href="'+doc.fileData+'" download="'+(doc.fileName||'document')+'" target="_blank">Download</a>'+
          '<a class="doc-link-btn" href="'+doc.fileData+'" target="_blank">View</a>':'')+
        (doc.link?'<a class="doc-link-btn" href="'+doc.link+'" target="_blank">Open Link</a>':'')+
        '<button class="doc-del-btn" onclick="deleteDoc(\''+doc._id+'\')">Delete</button>'+
      '</div>'+
    '</div>'
  ).join('');
}

function deleteDoc(id) {
  if (!confirm('Delete this document?')) return;
  deleteDocFromDb(id)
    .then(() => toast('Deleted.','#E24B4A'))
    .catch(err => toast('Error: '+err.message,'#E24B4A'));
}

/* ================================================================
   EXPORT CSV
   ================================================================ */
function exportCSV() {
  const md = monthDeliveries();
  if (!md.length) { toast('No data to export.','#E24B4A'); return; }
  const hdrs = ['Date','Time','DR No.','Plate No.','Supplier','Material',
    'Subtype','Source','Status','Tested By','Remarks'];
  const rows = md.map(d => [d.date,d.time,d.dr,d.plate,d.supplier,d.material,
    d.type||d.subtype,d.source,d.status,d.tester,d.remarks]
    .map(v => '"'+(v||'').replace(/"/g,'""')+'"').join(','));
  const csv = [hdrs.join(','),...rows].join('\n');
  const a = document.createElement('a');
  a.href = URL.createObjectURL(new Blob([csv],{type:'text/csv'}));
  const [y,m] = (selMonth()||'').split('-');
  a.download = 'BigBen_Asphalt_'+(y&&m?new Date(+y,+m-1,1).toLocaleDateString('en-PH',
    {year:'numeric',month:'long'}).replace(/ /g,'_'):'export')+'.csv';
  a.click();
  toast('CSV exported.','#378ADD');
}

function exportLayingCSV() {
  if (!layingEntries.length) { toast('No data to export.','#E24B4A'); return; }
  const accumMap    = buildAccumMap(layingEntries);
  const accumDurMap = buildAccumDurationMap(layingEntries);
  const hdrs = ['Date','Lab No.','Project ID','Project Title','Location','Client','JMF','Rate Type','Rate Result',
    'Load No.','Truck No.','Station From','Station To','Length (m)','Width (m)','Thickness (mm)','Lane No.','Lane Area',
    'Quantity (t)','Accumulative Qty (t)','Arrival Time','Laying Start','Laying End','Duration (min)','Accumulative Duration (min)',
    'Temp Arrival','Temp Laying','Temp Compaction','Temp Others Label','Temp Others Value','Layer'];
  const rows = layingEntries.map(d => {
    const dur = computeDurationMinutes(d.layingStart, d.layingEnd);
    return [d.date, d.labNo, d.projectId, d.projectTitle, d.projectLocation, d.client,
      d.jmf==='__other__'?d.jmfOther:d.jmf, d.rateType, d.rateResult,
      d.loadNo, d.truckNo, d.stationFrom, d.stationTo, d.length, d.width, d.thickness,
      d.laneNo, d.laneArea==='__other__'?d.laneAreaOther:d.laneArea,
      d.quantity, accumMap[d._id]!==undefined?accumMap[d._id].toFixed(2):'',
      d.arrivalTime, d.layingStart, d.layingEnd, dur!==null?dur:'',
      accumDurMap[d._id]!==undefined?accumDurMap[d._id]:'',
      d.tempArrival, d.tempLaying, d.tempCompaction, d.tempOthersLabel, d.tempOthersValue,
      d.layer==='__other__'?d.layerOther:d.layer]
      .map(v => '"'+String(v||'').replace(/"/g,'""')+'"').join(',');
  });
  const csv = [hdrs.join(','),...rows].join('\n');
  const a = document.createElement('a');
  a.href = URL.createObjectURL(new Blob([csv],{type:'text/csv'}));
  a.download = 'BigBen_AsphaltLaying_'+new Date().toISOString().slice(0,10)+'.csv';
  a.click();
  toast('CSV exported.','#378ADD');
}

/* ================================================================
   PRINT
   ================================================================ */
function printLog() {
  const logTab = document.querySelector('.tab:nth-child(2)');
  if (logTab) setTab('log', logTab);
  setTimeout(() => window.print(), 300);
}

function printLaying() {
  const layingTab = document.querySelector('.tab:nth-child(4)');
  if (layingTab) setTab('laying', layingTab);
  setTimeout(() => window.print(), 300);
}

/* ================================================================
   TOAST
   ================================================================ */
function toast(msg, color) {
  color = color||'#639922';
  const el  = document.getElementById('toast');
  const dot = document.getElementById('toast-dot');
  const msg_el = document.getElementById('toast-msg');
  if (!el) return;
  if (dot) dot.style.background = color;
  if (msg_el) msg_el.textContent = msg;
  el.classList.add('show');
  clearTimeout(el._t);
  el._t = setTimeout(() => el.classList.remove('show'), 3500);
}

/* ================================================================
   KEYBOARD
   ================================================================ */
document.addEventListener('keydown', function(e) {
  if (e.key==='Escape') { closeDeliveryModal(); closeDocForm(); closeLayingModal(); }
});

/* ================================================================
   INIT
   ================================================================ */
document.addEventListener('DOMContentLoaded', function() {
  const sel = document.getElementById('sel-month');
  if (sel) sel.addEventListener('change', function() {
    render();
    if (activeTab==='kpi') setTimeout(renderCharts,80);
  });

  loadLocal();
  buildMonthSelect();
  render();

  loadLocalLaying();
  buildLabFilterOptions();
  renderLayingLog();

  initFirebase();
  updateOnlineStatus();
  setTimeout(renderCharts, 200);
});
